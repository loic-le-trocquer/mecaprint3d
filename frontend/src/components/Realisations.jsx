...
function normalizeMedia(item) {
  const media = Array.isArray(item.media) ? item.media.filter((m) => m?.url) : [];
...
function RealisationCard({ item, index, onOpen }) {
          ...
          {mediaList.map((media, mediaIndex) => (
            <button
              ...
              className={`relative h-16 w-20 shrink-0 overflow-hidden rounded-xl border transition ${
                mediaIndex === activeIndex ? "border-orange-500" : "border-white/10 hover:border-orange-400"
              }`}
            >
              {media.type === "video" ? (
...
export default function Realisations({ content }) {
  ...
  const activeMedia = lightbox ? normalizeMedia(lightbox.item)[lightbox.index] : null;
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {realisations.map((item, index) => (
            <RealisationCard
              ...
              item={item}
              index={index}
              onOpen={(selectedItem, selectedIndex) => setLightbox({ item: selectedItem, index: selectedIndex })}
            />
          ))}
        </div>
      </div>
      ...
      {lightbox && activeMedia && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 backdrop-blur">
          <button
            ...
            className="absolute right-5 top-5 rounded-full border border-white/20 bg-white/10 px-4 py-2 font-black text-white hover:bg-orange-500"
          >
            Fermer
          </button>
          ...
          <div className="w-full max-w-6xl">
            ...
            <div className="mt-4 text-center">
              <p className="text-sm font-bold uppercase tracking-widest text-orange-400">{lightbox.item.category}</p>
              <h3 className="text-2xl font-black text-white">{lightbox.item.title}</h3>
            </div>
          </div>
        </div>
      )}
